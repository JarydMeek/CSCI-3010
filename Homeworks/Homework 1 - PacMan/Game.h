...
public:
	...
	std::vector<Position> GetMoves(Player *p, std::vector<std::string> *outputArr);
  ...
  // Move an enemy to a new position on the board. Return
	// true if they moved successfully, false otherwise.
  bool MoveEnemy(Player *p, Position pos, Player* actualPlayer);
...
public:
	...
	bool TakeTurnEnemy(Player *p, Player* actualPlayer, std::vector<Player*> *enemylist);
