    ...
    ui(new Ui::PlotWindow)
{
    ...
    connect(p2, &Point::DeletePoint, this, &PlotWindow::DeletePoint);
...
void PlotWindow::DrawLineSlot(Point *p) {
    if(shift_){
        scene->addLine((p->get_x()+ Point::get_width() / 2), (p->get_y()+ Point::get_width() / 2), (shift_->get_x()+ Point::get_width() / 2), (shift_->get_y())+ Point::get_width() / 2);
...
void PlotWindow::on_deletePoints_clicked()
{
    ...
    if(delete_mode_){
         ui->deletePoints->setStyleSheet("QPushButton{ background-color: red }");
    }
    else{
        ui->deletePoints->setStyleSheet("");
    }
}
